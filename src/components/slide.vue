    ...
    <slideheader  :titles='courseData.title' :text='text'/>
     <v-card style="padding:10px;  margin:0 20px;margin-top:20px; background-color:#eeeeee" v-if='courseData.signeddate'>
      ...
      <div  v-for='(slide, index) in courseData.slides[lang]' v-bind:key='index' v-scroll='handleScroll'   style="margin:0 20px;margin-bottom:40px" >
        <v-card v-if='get_checkpointpos()>index' style="padding:10px; margin:auto; margin-top:30px; background-color:#eeeeee"> <v-img  class="image_middle"  :src=slideurl?slideurl[index]:slide ref='images'/> </v-card>

        <v-card style="padding:10px;  margin-top:30px; background-color:#eeeeee" v-if='isInclude(0,index) &&get_checkpointpos()>=courseData.checkpoints[getcheckpointindex(index)].pos'>
        ...
        <v-card style="padding:10px;  margin-top:30px; background-color:#eeeeee" v-if='(isInclude(0,courseData.slides[lang].length)) &&(index===courseData.slides[lang].length-1)&&get_checkpointpos()>=courseData.checkpoints[getcheckpointindex(courseData.slides[lang].length)].pos '>
          <div class="checkpoint"  ref='checkpoints' >
        ...
        <v-card v-if='isInclude(1,index) && get_checkpointpos()>=courseData.questions[getquestionindex(index)].pos' style="padding:10px; margin-top:30px; background-color:#eeeeee">
  ...
  get_checkpointpos()
    {
      ...
      for(let i=0; i < courseData.checkpoints.length; i++)
          if(!this.courseData.checkpoints[i].checkedtime)
            pos =  this.courseData.checkpoints[i].pos;
      for(let i=0; i < courseData.questions.length; i++)
          if(!this.courseData.questions[i].score)
            if(pos <this.courseData.questions[i].pos)
              pos = this.courseData.questions[i].pos;
      return pos;
    },
    initslidepos(){
        ...
        if((bounds.bottom <=viewport.bottom) &&(bounds.top>= viewport.top))
          {
            ...
            this.slidepos = index;
            if( index ==this.courseData.slides.length-1)
            {
              if(this.before_finish == true)
              {
               this.finished = true;
               functionAPI.set_signedcourse(this.courseid);
              }
